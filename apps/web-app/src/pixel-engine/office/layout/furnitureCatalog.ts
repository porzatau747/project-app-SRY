...
const fallbackDeskSprite = Array(16).fill(Array(32).fill('#8B4513')) as SpriteData; // Brown desk
const fallbackChairSprite = Array(16).fill(Array(16).fill('#555555')) as SpriteData; // Gray chair
const fallbackPCSprite = Array(16).fill(Array(16).fill('#222222')) as SpriteData; // Dark gray PC
...
export function getCatalogEntry(type: string): CatalogEntryWithCategory | undefined {
  ...
  // Fallback for when assets are not loaded
  if (type === 'DESK_FRONT') {
    return { type, label: 'Desk', footprintW: 2, footprintH: 1, isDesk: true, category: 'desks', sprite: fallbackDeskSprite };
  }
  if (type === 'WOODEN_CHAIR_FRONT') {
    return { type, label: 'Chair', footprintW: 1, footprintH: 1, isDesk: false, category: 'chairs', sprite: fallbackChairSprite };
  }
  if (type === 'PC_FRONT_OFF') {
    return { type, label: 'PC', footprintW: 1, footprintH: 1, isDesk: false, category: 'electronics', canPlaceOnSurfaces: true, sprite: fallbackPCSprite };
  }
